...
public class UsersContacts {
    ...
    private APIContact initializeApiContact() {
        if (mApiContact == null) {
            mApiContact = this.mApiService.RootService(APIContact.class, PreferenceManager.getToken(mContext), EndPoints.BACKEND_BASE_URL);
        }
        return mApiContact;
    }
    ...
    public Observable<RegisterIDResponse> updateRegisteredId(String registeredId) {
        return initializeApiContact().updateRegisteredId(registeredId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .map(response -> response);
    }
